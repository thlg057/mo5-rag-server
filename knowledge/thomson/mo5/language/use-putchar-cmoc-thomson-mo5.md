# Use putchar() for character output in CMOC

## Goal
Output a single character using CMOC’s character-output path.

## Procedure
1. Call `putchar()` with the character to output.
...
- The manual also states the same redirection mechanism used for `printf()` output (via the `CHROUT` global variable) applies to `putchar()`.
